...
public class RedisService {
    ...
    * @return String
     */
    public String watchSet(String key, String field) {
        Jedis jedis = null;
        boolean isBroken = false;
        try {
            jedis = this.getJedis();
            jedis.watch(App.Redis.GET_ORDER_CODE);
            String numString = jedis.hget(App.Redis.GET_ORDER_CODE, field);
            String values = null;
            int a = 0;
            ...
            if (a == 1) {
                //初始化订单明细编码
                jedis.hset(App.Redis.GET_ORDER_CODE, "order_detail", "0");
            }
            return values;
        }
        catch (Exception e) {
    ...
    * @param seconds    过期秒数
    ...
    public void setex(Object key, Object value, int seconds) {
        ...
        try {
            jedis = this.getJedis();
            System.out.println(jedis.setex(key.toString(), seconds, value.toString()));
        }
        catch (Exception e) {
            e.printStackTrace();
            isBroken = true;
    ...
    * @param field      字段键
    ...
    /**
     * 批量删除多个哈希域。从 key 指定的哈希集中移除指定的域。在哈希集中不存在的域将被忽略。如果 key 指定的哈希集不存在，它将被认为是一个空的哈希集，该命令将返回0。
    * @author 张进军
    * @date Dec 25, 2015 9:57:31 PM
    * @param key	hash键
    * @param field	字段集合
     */
    ...
    * @return String
     */
    public String watchSet(String key) {
        Jedis jedis = null;
        boolean isBroken = false;
        try {
            jedis = this.getJedis();
            jedis.watch(App.Redis.GET_ORDER_CODE);
            String numString = jedis.get(App.Redis.GET_ORDER_CODE);
