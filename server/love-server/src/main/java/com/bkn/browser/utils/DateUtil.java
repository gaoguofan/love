...
public class DateUtil {
	...
	public static Date tranStrToDate2(String dataStr) {
        try {
	...
	* @return String
	 ...
	 * @return String
	 */
	public static String minusSecondes(String dateTime, int secondes) {
		try {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			Calendar date = Calendar.getInstance();
			date.setTime(sdf.parse(dateTime));
    ...
    public static String getRandomDate() {
        ...
        currentDate.add(GregorianCalendar.DATE, mondayPlus + 6);
        Date monday = currentDate.getTime();
        DateFormat df = new SimpleDateFormat("dHHmmssS");
        String preMonday = df.format(monday);
        return preMonday.substring(1, preMonday.length());
    }
	...
	* @throws ParseException
	 */
	public static int daysBetweenMinutes(String smdate, String bdate) {
		try {
    ...
    public static String subJd(String date) {
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
            Date dt = sdf.parse(date);
            Calendar rightNow = Calendar.getInstance();
    ...
    public static String subDay(String date, int dayCount) {
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
            Date dt = sdf.parse(date);
            Calendar rightNow = Calendar.getInstance();
            rightNow.setTime(dt);
            rightNow.add(Calendar.DATE, dayCount);
            Date dt1 = rightNow.getTime();
            String reStr = sdf.format(dt1);
            return reStr;
        } catch (Exception e) {
	 ...
	 *  返回一个月前的时间。
	* @author 骆峰
	* @date 2016年10月31日 下午4:17:15
	* @return String
	* @throws ParseException
	 */
	public static String subtractMonth() {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        Calendar rightNow = Calendar.getInstance();
	 ...
	 * 
	 * @param beginTime
	 *            开始时间(eg: "2015-12-19 17:52:03")
	 * @param endTime
	 *            结束时间(eg: "2015-12-19 20:52:03")
	 * @return (分钟)时间差
	 */
	...
	* @return
	 */
	public static int getTwoTimeBetweenSecond(String beginTime, String endTime) {
        SimpleDateFormat myFormatter = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        long minute = 0;
        try {
            Date beginDate = myFormatter.parse(beginTime);
            Date endDate = myFormatter.parse(endTime);
            minute = (endDate.getTime() - beginDate.getTime()) / 1000;
        } 
        catch (ParseException e) {
            e.printStackTrace();
        }
	...
	public static String getWeekOfDate(Date date) {
		/*
		 * SimpleDateFormat sdf = new SimpleDateFormat("EEEE"); String week =
		 * sdf.format(date); return week;
		 */
	...
	public static String caculateSeniority(String employeeCreateTime) throws ParseException {
		...
		int m = java.lang.Math.abs(month - month2); // 得到月差
		return t + "年" + m + "月";
	}

	/**
	 * 得到当前日期前后 ?天 的日期，格式为Map<"yyyy-MM-dd", "EEEE">
	 * @author DavidLiang
	 * @date 2016年1月31日 下午8:33:16
	 * @param dayNum       天数
	 * @return 当前日期前后 ?天 的日期
	 */
	public static List<Map<String, String>> getDateBetweenSixty(int dayNum) {
		/*
		 * Map<String, String> dateMap = new TreeMap<String, String>();
		 * SimpleDateFormat dayFormat = new SimpleDateFormat("MM-dd");
		 * SimpleDateFormat weekFormat = new SimpleDateFormat("EEEE"); Calendar
		 * calendar = Calendar.getInstance(); calendar.setTime(new Date());
		 * //当前calendar对象(当前日期)加上指定天数 calendar.add(Calendar.DATE, 30); for (int
		 * i=0; i<30; i++) { calendar.add(Calendar.DATE, -1); String customeDay
		 * = dayFormat.format(calendar.getTime()); String customeWeek =
		 * weekFormat.format(calendar.getTime()); dateMap.put(customeDay,
		 * customeWeek); }
		 */
		...
		Map<String, String> dateMap = new TreeMap<String, String>();
		dateMap.put("date", nowDay);
		dateMap.put("week", weeks[nowWeekIndex]);
		...
		for (int i = 0; i < dayNum; i++) {
			...
			date.put("date", customeDay);
			date.put("week", weeks[customeWeekIndex]);
			dateList.add(date);
		}
		return dateList;
	}
	...
	/**
	 * 根据星期几返回员工班次id字段 eg:星期一返回"shif_ida"...
	 * 
	 * @author DavidLiang
	 ...
	 *            时间格式
	 * @param returnFormat
	 *            返回格式(day:返回相隔多少天; hour:返回相隔多少小时; min:返回相隔多少分钟; sec:返回相隔多少秒)
	 * @return 时间差
	 */
	public static Long dateDiff(String startTime, String endTime, String dateFormat, String returnFormat) {
		...
		try {
			...
			/*
			 * diffHour = diff % nd / nh + diffDay * 24; // 计算差多少小时 diffMin =
			 * diff % nd % nh / nm + diffDay * 24 * 60; // 计算差多少分钟 diffSec =
			 * diff % nd % nh % nm / ns; // 计算差多少秒
			 */
	...
	/**
	 * 根据班次判断是否跨日 eg: 23:00, 06:30就涉及跨日
	 * 
	 * @author DavidLiang
