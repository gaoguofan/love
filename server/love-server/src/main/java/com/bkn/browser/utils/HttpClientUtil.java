...
public class HttpClientUtil {
    ...
    /**
     * 发送GET请求,基于保存的cookie进行访问
     * @param url 请求地址
     * @param charset 返回数据编码
     * @param cookieStore cookie信息
     * @return 返回数据
     */
    public static String sendGetReqByCookies(final String url, String charset, BasicCookieStore cookieStore) {
        if (null == charset) {
            charset = DEFAULT_CHARSET;
        }
    ...
    * @return   响应数据
    ...
    public static String sendPostReq(String url, Map<String, ?> params, String charset) {
        return sendPostReq(url, params, charset, null);
    }
     ...
     * @return 返回数据
     */
    public static String sendPostReq(String url, Map<String, ?> params, String charset, BasicCookieStore cookieStore) {
        if (null == charset){
            charset = DEFAULT_CHARSET;
        }
        ...
        try {
            ...
            if ((statuscode == HttpStatus.SC_MOVED_TEMPORARILY) || (statuscode == HttpStatus.SC_MOVED_PERMANENTLY)
                ...
                if (header != null) {
                    ...
                    if (cookieStore!=null){
                        cookieStore = setCookieStore(httpResponse, cookieStore, httpPost.getURI().getHost());
                        return sendGetReqByCookies(newuri, null, cookieStore);
                    }
                    return sendGetReq(newuri, null);
                }
            }
    ...
    public static byte[] downloadPost(String url, Map<String, String> params) {
        ...
        try {
            ...
            if (entity.getContentType().getValue().equals("application/json")) {
                logger.error("downloadPost error : " + EntityUtils.toString(entity, charset));
                return null;
            }
    ...
    * @return
     */
    public static String sendPostReq(String url, String requestBody) {
        CloseableHttpClient httpClient = HttpClientBuilder.create().build();
        RequestConfig reqConf = RequestConfig.DEFAULT;
        HttpPost httpPost = new HttpPost(url);
        try {
            ...
            logger.error("不支持当前参数编码格式[" + DEFAULT_CHARSET + "],堆栈信息如下", e);
        }
        catch (ClientProtocolException e) {
            logger.error("协议异常,堆栈信息如下", e);
        }
    ...
    * @return                   cookie集合
     */
    public static BasicCookieStore setCookieStore(HttpResponse httpResponse, BasicCookieStore cookieStore, String host) {
